  ...
  try {
    ...
    stage('Validate') {
      ...
      for (int i = 0; i < packer_files.size(); i++) {
        ...
        sh "packer validate ${packer_file.path}"
      }
    }
      ...
      for (int i = 0; i < packer_files.size(); i++) {
          ...
          stepsForParallel[stepName] = {
            node {
              ...
              sh "packer build --var destiny_artifacts=${params.destiny_artifacts} --var docker_image_name=${modified_image_mame} --var docker_image_tag=${docker_tag} ${packer_file.path}"
            }
          }
      }
